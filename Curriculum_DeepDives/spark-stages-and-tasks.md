...
┌─────────────────────────────────┐ ┌──────────────────────────────────────────────┐
│ Spark Session / User Code │ │ Executor Thread Pool │
│ │ │ │ ┌────────────────────────────────────────┐ │
│ ▼ │ │ │ Task 1 (Partition 0) │ │
│ Catalyst Optimizer │ │ │ ┌────────────────────────────────────┐ │ │
│ (Logical & Physical Planning) │ │ │ │ Tungsten Whole-Stage CodeGen │ │ │
│ │ │ │ │ │ (Off-heap memory manipulation) │ │ │
│ ▼ │ Task Dispatch │ │ └────────────────────────────────────┘ │ │
│ DAGScheduler │────────────────▶│ └────────────────────────────────────────┘ │
│ (Stage creation at boundaries) │ (Serialized) │ ┌────────────────────────────────────────┐ │
│ │ │ │ │ Task 2 (Partition 1) │ │
│ ▼ │ │ │ ┌────────────────────────────────────┐ │ │
│ TaskScheduler │ │ │ │ Tungsten Whole-Stage CodeGen │ │ │
│ (Task assignment & locality) │ │ │ │ (Off-heap memory manipulation) │ │ │
└─────────────────────────────────┘ │ │ └────────────────────────────────────┘ │ │
 │ └────────────────────────────────────────┘ │
 │ BlockManager / ShuffleService │
...
- **ResultTask:** A task that computes a final materialized result and sends the data back to the Driver (e.g., in terminal operations like `collect()`, `count()`, or `show()`). It always represents the terminal point of an execution DAG. [Ref: 452](spark_book.pdf#page=452)
...
| `groupBy`.`agg` | O(N) | Yes | Hash aggregate. Can perform partial aggregation in the map stage before the shuffle to reduce network payload. | [Ref: 462](spark_book.pdf#page=462)
