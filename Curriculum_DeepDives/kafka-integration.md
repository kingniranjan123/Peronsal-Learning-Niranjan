...
- **KafkaDataConsumer:** The executor-side component that manages the internal KafkaConsumer instances, pooling them to avoid the significant overhead of establishing new TCP connections for every micro-batch. [Ref: 452](spark_book.pdf#page=452)
...
| Offset Fetch (Driver) | O(P) | No | Extremely fast metadata operation, where P is the number of partitions. Handled entirely by the Driver. | [Ref: 463](spark_book.pdf#page=463)
